...
class Touch {
  constructor(event, touch, { context = TouchContext.PRIMARY_CLICK } = {}) {
    ...
    this.world = canvas.stage.transform.worldTransform.applyInverse({ x: this.current.x, y: this.current.y });  //< Position in the world where the user touched
  ...
  update(event, touch) {
    ...
    this.movement = this.add(this.movement, this.subtract(this.current, this.last));
  }
