...
import {
  installDrawingToolsControls,
  installMeasurementTemplateEraser,
  initEraserTools,
} from "./eraserTools";
import {
  findCanvas,
...
export const initClientTools = () => {
  ...
  Hooks.on("ready", function () {
    try {
      ...
      if (canvasElement) {
        ...
        const canvasTouchToMouseAdapter =
          CanvasTouchToMouseAdapter.init(canvasElement);
        canvasTouchWrapper(canvasTouchToMouseAdapter);
      } else {
        console.warn(
          `Failed to find canvas element. ${MODULE_DISPLAY_NAME} touch settings will not be available.`,
        );
      }
    } catch (e) {
      console.error(
        `Failed to initialize ${MODULE_DISPLAY_NAME} touch settings: `,
        e,
      );
    }
  });
  ...
  Hooks.on("canvasReady", function () {
    ...
    const mgr = new CustomMouseInteractionManager(
      canvas.stage,
      canvas.stage,
      { clickRight2: false },
      callbacks,
